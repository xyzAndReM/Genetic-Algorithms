...
function ProblemaDoCaixeiroViajante(NumeroDeCidades,Ncrom,Niter)
...
tamanhoPopulacao = Ncrom; % Tamanho inicial da população
...
format short g
disp(['Tamanho da População = ' num2str(tamanhoPopulacao)])
disp(['Taxa de Mutação = ' num2str(taxaMutacao)])
disp(['Número de cidades = ' num2str(Ncidades)])
disp(['Meror custo: ' num2str(fitness(1))])
disp(['Trajeto ideal: ']); disp([num2str(populacao(1,:))])

%Imprime o caminho em um mapa.
figure(1);
plot([x(populacao(1,:)) x(populacao(1,1))],[y(populacao(1,:)) y(populacao(1,1))],x,y,'*');

%Imprime as estatística: o menor valor de fitness e o valor de fitness 
% médio para cada geração
figure(2)
iteracao= 1:MaxIteracoes;
plot(iteracao,menorCusto,'--',iteracao,mediaCusto);
xlabel('Geração');
ylabel('Fitness');
